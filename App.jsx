...
/** Different GeoJSON datasets use different keys for the country name.
 *  This handles all common conventions so every source works without
 *  per-source special-casing. */
const getFeatureName = p => p?.name || p?.ADMIN || p?.NAME || "";
...
/* ─── Ordered fallback GeoJSON sources ─────────────────────────────
   All four serve valid GeoJSON FeatureCollections with CORS enabled.
   Property name conventions differ (see getFeatureName below):
     1 & 3 → properties.name   (D3 gallery / PublicaMundi schema)
     2     → properties.ADMIN  (Natural Earth / geojson.xyz schema)
     4     → properties.name   (GitHub raw backup)
──────────────────────────────────────────────────────────────────── */
const GEO_SOURCES = [
...
export default function App() {
  ...
  /* Reactive window dimensions – projection re-computes on any resize */
  ...
  const loadMap = useCallback(async () => {
    ...
    for (let i = 0; i < GEO_SOURCES.length; i++) {
      ...
      try {
        ...
        const tid  = setTimeout(() => ctrl.abort(), 20000);   // 20 s – tolerates slow / mobile networks
        const r    = await fetch(GEO_SOURCES[i], { signal: ctrl.signal, mode: "cors" });
        clearTimeout(tid);
        if (!r.ok) continue;
  ...
  useEffect(() => {
    const h = () => {
      ...
      setWinW(window.innerWidth);   // triggers projection recompute
              ...
              {worldGeo && (
                <>
                  ...
                  {worldGeo.features.map(f => (
                    <path
                      key={getFeatureName(f.properties) || String(f.id ?? f.properties?.iso_a3)}
                      className="country-path"
                      d={pathGen(f) || ""}
                      fill={getCountryFill(getFeatureName(f.properties))}
                      stroke="#2e2012"
                      strokeWidth={.4}
                      onClick={() => handleCountryClick(getFeatureName(f.properties))}
                    />
                  ))}
                  {CONFLICTS.map(c => {
